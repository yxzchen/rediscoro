...
///
/// Example timeline:
///   t=0: Error detected → FAILED
///   t=0: Attempt 1 (immediate) → fail
///   t=0: Attempt 2 (immediate) → fail
///   ...
///   t=0: Attempt 5 (immediate) → fail
///   t=0: Enter backoff phase, state = FAILED
///   t=100ms: RECONNECTING → Attempt 6 → fail
///   t=100ms: state = FAILED (sleep 200ms)
///   t=300ms: RECONNECTING → Attempt 7 → fail
///   t=300ms: state = FAILED (sleep 400ms)
///   ...
///   t=X: delay reaches max_delay (30s)
///   t=X: Keep retrying every 30s indefinitely
