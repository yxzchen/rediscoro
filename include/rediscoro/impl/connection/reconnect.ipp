...
inline auto connection::do_reconnect() -> iocoro::awaitable<void> {
  ...
  REDISCORO_LOG_DEBUG("reconnect loop start: state={}", to_string(state_));
  while (!tok.stop_requested() && state_ != connection_state::CLOSED) {
    ...
    REDISCORO_LOG_INFO("reconnect attempt: index={} delay_ms={} generation={}",
    ...
    if (tok.stop_requested() || state_ == connection_state::CLOSING) {
      REDISCORO_LOG_INFO("reconnect cancelled: state={}", to_string(state_));
      co_return;
    }

    // Attempt reconnect.
    REDISCORO_LOG_INFO("state transition: reason=reconnect_attempt from={} to={}",
    ...
    if (!reconnect_res) {
      ...
      REDISCORO_LOG_WARNING("reconnect failed: err_code={} err_msg={} detail={}",
                            reconnect_res.error().code.value(),
                            reconnect_res.error().code.message(), reconnect_res.error().detail);
      REDISCORO_LOG_INFO("state transition: reason=reconnect_failed from={} to={}",
                         to_string(connection_state::RECONNECTING),
                         to_string(connection_state::FAILED));
    ...
    reconnect_count_ = 0;
    REDISCORO_LOG_INFO("reconnect succeeded: generation={}", generation_);
    read_wakeup_.notify();
