...
auto run_session(int session_id, bench_state* st) -> asio::awaitable<void> {
  ...
  while (remaining > 0) {
    ...
    if (ec) {
      fail_and_stop(st, "boostredis_redis_pipeline_depth: PING failed for session " +
...
int main(int argc, char* argv[]) {
  ...
  if (sessions <= 0) {
    std::cerr << "boostredis_redis_pipeline_depth: sessions must be > 0\n";
    return 1;
  }
  if (total_cmds_per_session == 0) {
    std::cerr << "boostredis_redis_pipeline_depth: total_cmds_per_session must be > 0\n";
    return 1;
  }
  if (pipeline_depth == 0) {
    std::cerr << "boostredis_redis_pipeline_depth: pipeline_depth must be > 0\n";
    return 1;
  }
  ...
  if (st.remaining_sessions.load(std::memory_order_acquire) != 0) {
    std::cerr << "boostredis_redis_pipeline_depth: incomplete run (remaining_sessions="
  ...
  if (sample_count != total_cmds) {
    std::cerr << "boostredis_redis_pipeline_depth: sample mismatch (expected=" << total_cmds
              << ", got=" << sample_count << ")\n";
    return 1;
  }
  ...
  std::cout << "boostredis_redis_pipeline_depth"
            << " host=" << st.host
            << " port=" << st.port
            << " sessions=" << sessions
